...
DWORD joytype[2] = { DEVICE_JOYSTICK, DEVICE_JOYSTICK };	// Emulation Type for joysticks #0 & #1
...
void CheckWiiMote0()
{
	...
	lastcheck = currtime;
		...
		//wiimote is held up	       
		//keydown[1] = (wd->btns_h & WPAD_BUTTON_DOWN);		
//		keydown[3] = (wd->btns_h & WPAD_BUTTON_LEFT);
//		keydown[5] = (wd->btns_h & WPAD_BUTTON_RIGHT);
//		keydown[7] = (wd->btns_h & WPAD_BUTTON_UP);
		//wiimote is held sideways

		keydown[1] = (wd->btns_h & WPAD_BUTTON_LEFT);		
		keydown[3] = (wd->btns_h & WPAD_BUTTON_UP);
		keydown[5] = (wd->btns_h & WPAD_BUTTON_DOWN);
		keydown[7] = (wd->btns_h & WPAD_BUTTON_RIGHT);
		...
		if (down) {
			...
			if (xkeys)
				...
				xpos[0] = PDL_CENTRAL + g_nPdlTrimX;
			if (ykeys)
				ypos[0] = ytotal / ykeys;
			else
				ypos[0] = PDL_CENTRAL + g_nPdlTrimY;	
...
void CheckJoystick0()
{
	...
	if ((SDL_JoystickGetButton(joy1, 0))) {
		gamecube_joypad_enabled = 1;		
	}

	if (!gamecube_joypad_enabled) return;
...
//    {
		...
		if ((SDL_JoystickGetButton(joy1, 0)) && !joybutton[0])
			buttonlatch[0] = BUTTONTIME;
		if ((SDL_JoystickGetButton(joy1, 1)) && !joybutton[1] && (joyinfo[joytype[1]].device == DEVICE_NONE)	// Only consider 2nd button if NOT emulating a 2nd Apple joystick
		    )
			buttonlatch[1] = BUTTONTIME;
...
//      {
		...
		xpos[1] =
		    (SDL_JoystickGetAxis(joy2, 0) -
		     joysubx[1]) >> joyshrx[1];
		ypos[1] =
		    (SDL_JoystickGetAxis(joy2, 1) -
		     joysuby[1]) >> joyshry[1];
...
//    JOYCAPS caps;
		if (number_of_joysticks > 0)	//(joyGetDevCaps(JOYSTICKID1,&caps,sizeof(JOYCAPS)) == JOYERR_NOERROR)
		{
			...
			UINT xrange =
			    AXIS_MAX /*caps.wXmax- */  - AXIS_MIN;
			UINT yrange =
			    AXIS_MAX /*caps.wYmax- */  - AXIS_MIN;
			while (xrange > 256) {
				xrange >>= 1;
...
BOOL JoyProcessKey(int virtkey, BOOL extended, BOOL down, BOOL autorep)
{
	...
	int nJoyNum =
	    (joyinfo[joytype[0]].device == DEVICE_KEYBOARD) ? 0 : 1;
	...
	if (isALT /*virtkey == VK_MENU */ )	// VK_MENU == ALT Key (Button #0 or #1)
	{
		...
		if ((virtkey >= SDLK_KP1) && (virtkey <= SDLK_KP9)) {
			...
			switch (virtkey) {	//Here VK_PRIOR means “Page Up”, VK_NEXT is “Page Down”
			case SDLK_END:
				keydown[0] = down;
				break;
			case SDLK_DOWN:
		...
		if ((virtkey == SDLK_KP0) || (virtkey == SDLK_INSERT)) {
			if (down) {
				if (joyinfo[joytype[1]].device !=
				    DEVICE_KEYBOARD) {
					buttonlatch[0] = BUTTONTIME;
				} else if (joyinfo[joytype[1]].device !=
					   DEVICE_NONE) {
					buttonlatch[2] = BUTTONTIME;
					buttonlatch[1] = BUTTONTIME;	// Re-map this button when emulating a 2nd Apple joystick
				}
			}
		} else if ((virtkey == SDLK_KP_PERIOD)
			   || (virtkey == SDLK_DELETE)) {
			if (down) {
				if (joyinfo[joytype[1]].device !=
				    DEVICE_KEYBOARD)
...
//===========================================================================
BYTE /*__stdcall*/ JoyResetPosition(WORD, WORD, BYTE, BYTE,
				    ULONG nCyclesLeft)
{
	CpuCalcCycles(nCyclesLeft);
...
/*      MessageBox(window,
			...
			fprintf(stderr,
				"Can not find joystick #%d - disabling it\n",
				nJoystickNumber);
			return 0;
		}
	} else if ((joyinfo[newtype].device == DEVICE_MOUSE) &&
		   (joyinfo[joytype[nJoystickNumber]].device !=
		    DEVICE_MOUSE)) {
		if (sg_Mouse.Active()) {
			/*MessageBox(window,
		...
		printf
		    ("To release mouse cursor, press Ctrl + left mouse button.\n");
	}
