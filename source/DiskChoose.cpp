...
int getstat(char *catalog, char *fname, int * size)
{
	...
	if(S_ISDIR(info.st_mode)) return 1;	// seems to be direcotiry
...
// prepare screen
	...
	//surface_fader(my_screen, 0.3F, 0.3F, 0.3F, -1, 0);	// fade it out to 30% of normal
	...
	while(true)
	{
		...
		font_print_centered(sx/2 ,5 , incoming_dir, screen, 1.5, 1.3);
		if (slot == 6) font_print_centered(sx/2,20,"Choose image for floppy 140KB drive", screen, 1, 1);
		else
			if (slot == 7) font_print_centered(sx/2,20,"Choose image for Hard Disk", screen, 1, 1);
		else
			if (slot == 5) font_print_centered(sx/2,20,"Choose image for floppy 800KB drive", screen, 1, 1);
		else
			if (slot == 1) font_print_centered(sx/2,20,"Select file name for saving snapshot", screen, 1, 1);
		else
			if (slot == 0) font_print_centered(sx/2,20,"Select snapshot file name for loading", screen, 1, 1);



		font_print_centered(sx/2,30, "Press ENTER to choose, or ESC to cancel",screen, 1.4, 1.1);
...
//				char tmp2[80],tmp3[256];
				...
				if (i == act_file) { // show item under cursor (in inverse mode)
					SDL_Rect r;
					r.x= 2;
					r.y= TOPX + (i-first_file) * 15 - 1;
					if(strlen(tmp) > 46) r.w = 46 * 6 * 1.7 + 2;
					   else r.w= strlen(tmp) * 6 * 1.7 + 2;	// 6- FONT_SIZE_X
					r.h= 9 * 1.5;
				...
				font_print(4, TOPX + (i - first_file) * 15, tmp, screen, 1.7, 1.5); // show name
				font_print(sx - 70, TOPX + (i - first_file) * 15, siz, screen, 1.7, 1.5);// show info (dir or size)
