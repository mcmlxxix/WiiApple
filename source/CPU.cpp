...
mutex_t g_CriticalSection;


static void pthread_mutex_lock(mutex_t *i)
{
	LWP_MutexLock(*i);
}

static void pthread_mutex_unlock(mutex_t *i)
{
	LWP_MutexUnlock(*i);
}
...
#define ABS	 addr = efix(*(LPWORD)(mem+regs.pc));	 regs.pc += 2;
#define IABSX    addr = efix(*(LPWORD)(mem+(efix(*(LPWORD)(mem+regs.pc)))+(WORD)regs.x)); regs.pc += 2;
#define ABSX	 base = efix(*(LPWORD)(mem+regs.pc)); addr = base+(WORD)regs.x; regs.pc += 2; CHECK_PAGE_CHANGE;
#define ABSY	 base = efix(*(LPWORD)(mem+regs.pc)); addr = base+(WORD)regs.y; regs.pc += 2; CHECK_PAGE_CHANGE;
#define IABSCMOS base = efix(*(LPWORD)(mem+regs.pc));			\
	addr = efix(*(LPWORD)(mem+base));				\
		 if ((base & 0xFF) == 0xFF) uExtraCycles=1;		  \
		 regs.pc += 2;
#define IABSNMOS base = efix(*(LPWORD)(mem+regs.pc));			\
		 if ((base & 0xFF) == 0xFF)				  \
...
#define INDY	 if (*(mem+regs.pc) == 0xFF)                         \
			 ...
			 base = efix(*(LPWORD)(mem+*(mem+regs.pc))); \
		 regs.pc++;                                          \
...
#define BRK	 regs.pc++;						    \
		 ...
		 regs.pc = efix(*(LPWORD)(mem+0xFFFE));
...
#if 0
		if (debug) {
			fprintf(debug, "Fetch %02x %08lx\n", iOpcode, uExecutedCycles);
			fclose(debug);
		}
#endif
...
//	InitializeCriticalSection(&g_CriticalSection);
	LWP_MutexInit (&g_CriticalSection, false);

#if 0

	if (debug) {
		fprintf(debug, "xRegs.pc %08lx (%d)\n", regs.pc, (regs.pc & 0xF000) == 0xC000);
	}
#endif
