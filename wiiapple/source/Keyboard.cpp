...
//	g_bAltKey   = (GetKeyState( VK_MENU   ) & KF_UP) ? true : false;
	...
	g_bShiftKey = (mstate & KMOD_RSHIFT) | (mstate & KMOD_LSHIFT);
	g_bCtrlKey  = (mstate & KMOD_RCTRL) | (mstate & KMOD_LCTRL);
	g_bAltKey   = (mstate & KMOD_RALT) | (mstate & KMOD_LALT);
...
void KeybToggleCapsLock ()
{
	if (!IS_APPLE2)
	{
		...
		//g_bCapsLock = SDL_GetModState() & KMOD_CAPS;
